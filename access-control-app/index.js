...
eventEmitter.on("receivedLog", async (parsedEventLog) => {
  ...
  try {
    ...
    if (!attendance) {
      const result = await queryAsync(
        "INSERT INTO attendances (uid, employee_id, machine_id, attendance_method_id, site_id, type, timestamp, longitude, latitude) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
        [
          eventData.serialNo,
          employeeId,
          machine.id,
      ...
      logger.info("Attendance already exists for UID " + eventData.serialNo);
    }
  } catch (error) {
